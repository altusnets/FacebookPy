...
SQL_CREATE_INVITE_RESTRICTION_TABLE = """
    CREATE TABLE IF NOT EXISTS `inviteRestriction` (
        `profile_id` INTEGER REFERENCES `profiles` (id),
...
def create_tables(cursor, tables):
    ...
    if "inviteRestriction" in tables:
        cursor.execute(SQL_CREATE_INVITE_RESTRICTION_TABLE)
