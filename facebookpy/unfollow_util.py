...
def follow_user(
    ...
    if track in ["profile", "post"]:
        ...
        following_status, follow_button = get_following_status(
            browser, track, login, userid_to_follow, None, logger, logfolder
        )
        if following_status in ["Follow", "Follow Back"]:
            ...
            logger.warning(
                "--> Couldn't follow '{}'!\t~{}".format(userid_to_follow, failure_msg)
            )
    ...
    if blacklist["enabled"] is True:
        ...
        add_user_to_blacklist(
            userid_to_follow, blacklist["campaign"], action, logger, logfolder
        )
...
def get_given_user_followers(
    ...
    allfollowers, allfollowing = get_relationship_counts(
        browser, "https://www.facebook.com/", user_name, userid, logger, Settings
    )
    ...
    try:
        ...
        for followers_link in followers_links:
            splitted = (
                followers_link.get_attribute("href")
